...
class UntargetedLKDObjective(Objective):
    """
    Untargeted likelihood objective.
    """
    ...
    def __call__(self, logits, perturbations=None):
        ...
        if logits.size(1) > 1:
            dists = -2 * logits
            dists_negs = dists.scatter(1, self.true_classes.unsqueeze(1), torch.max(dists, dim=1, keepdim=True)[0])
