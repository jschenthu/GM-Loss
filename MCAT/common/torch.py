...
def gm_cross_entropy_divergence_lkd(logits, weights, labels, dthres, alpha=0.1, lambdaa=0.01, beta=1.0, wc=1.0, nogt = False, logits_clean = None, reduction='mean'):
    """
    Loss.
    ...
    loss_nat = loss_ce + loss_lkd * lambdaa
    if logits_clean is not None:
        if nogt:
            dists_negs = dists.scatter(1, labels.unsqueeze(1), torch.max(dists, dim=1, keepdim=True)[0])
        else:
            dists_negs = dists
        likelihood = torch.min(dists_negs, dim=1)[0]
        #dists_clean = logits_clean * (-2)
        #likelihood_clean = torch.min(dists_clean, dim=1)[0]
        loss_adv = torch.clamp(dthres - likelihood, min=0)
        loss = loss_nat * (1 - weights) + weights * loss_adv * beta
    else:
        likelihood = torch.min(dists, dim=1)[0]
        loss = loss_nat + likelihood * wc * beta
