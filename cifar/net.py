...
class VGG(nn.Module):
    ...
    def __init__(
        ...
        self.cfg = [96, 96, 96, 96, 96, 'M', 192, 192, 192, 192, 'M', 384, 384, 384, 384]
        ...
        if not gm:
            ...
            self.linear = LGM_Linear(384, num_classes)
            nn.init.kaiming_normal_(self.linear.mu, gain)
    ...
    def _initialize_weights(self, gain) -> None:
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, gm=True):
        ...
        if not gm:
            ...
            logits = LGM_Linear(64, num_classes)
...
def resnet20(gm=True, num_classes=10):
    return ResNet(BasicBlock, [3, 3, 3], gm=gm, num_classes=num_classes)


def resnet32(gm=True):
    return ResNet(BasicBlock, [5, 5, 5], gm=gm)


def resnet44(gm=True):
    return ResNet(BasicBlock, [7, 7, 7], gm=gm)


def resnet56(gm=True):
    return ResNet(BasicBlock, [9, 9, 9], gm=gm)


def resnet110(gm=True):
    return ResNet(BasicBlock, [18, 18, 18], gm=gm)
