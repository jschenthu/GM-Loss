...
class PartialFC(Module):
    ...
    def __init__(self, rank, local_rank, world_size, batch_size, resume,
                 margin_softmax, num_classes, lambdaa=0.001, sample_rate=1.0, embedding_size=512, prefix="./"):
        """
        rank: int
    ...
    def save_params(self, global_step):
        """ Save softmax weight for each rank on prefix
        """
        torch.save(self.weight.data, self.weight_name[:-3] + '_{}.pt'.format(global_step))
        torch.save(self.weight_mom, self.weight_mom_name[:-3] + '_{}.pt'.format(global_step))
    ...
    def forward(self, total_features, norm_weight):
        ...
        xy = torch.matmul(total_features, norm_weight.permute(1, 0))
        xx = torch.sum(total_features * total_features, dim=1, keepdim=True)
        yy = torch.sum(norm_weight * norm_weight, dim=1, keepdim=True).permute(1, 0)
        logits = -0.5 * (xx - 2.0 * xy + yy)
        return logits
    ...
    def forward_backward(self, label, features, optimizer):
        ...
        logits_ori = self.forward(total_features, norm_weight)
        logits, margin = self.margin_softmax(logits_ori, total_label)
        ...
        with torch.no_grad():
            ...
            loss_lkd = (-2 * logits_ori).mean(dim=1, keepdim=True) / self.world_size
            dist.all_reduce(loss_lkd, dist.ReduceOp.SUM)
            loss_lkd_v = loss_lkd.mean()

            # calculate grad
            grad[index] -= one_hot
            grad -= 2 * self.lambdaa / self.num_classes
            grad[index] += 2 * self.lambdaa / self.num_classes * (1 - margin)
