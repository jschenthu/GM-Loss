...
class CallBackModelCheckpoint(object):
    ...
    def __call__(self, global_step, backbone, partial_fc, opt_backbone, opt_pfc):
        if global_step > 100 and self.rank == 0:
            path_module = os.path.join(self.output, "backbone_{}.pth".format(global_step))
            path_opt1 = os.path.join(self.output, 'opt_backbone_{}.pth'.format(global_step))
            path_opt2 = os.path.join(self.output, 'opt_pfc_{}.pth'.format(global_step))
            torch.save(opt_backbone.state_dict(), path_opt1)
            torch.save(opt_pfc.state_dict(), path_opt2)
